...
import java.net.URL;
public class EnergyGuiApplication
        extends Application {
    private TextArea responseArea = new TextArea();

    @Override
    public void start(Stage primaryStage) {
        primaryStage.setTitle("Energy GUI - Community vs. Grid");        // Current Data Button
        Button getCurrentBtn = new Button("Get Current Data");
        getCurrentBtn.setOnAction(e -> {
            String response = getApiResponse("http://localhost:8080/api/energy/current");
            responseArea.setText(response);        });        // Historical Data Controls
            ...
            getHistoricalBtn.setOnAction(e -> {
                ...
                responseArea.setText(response);        });        // Layout
